suppressMessages({
  library(xts); library(data.table); library(pbapply)
})
...
build_signals <- function(dat, tf_minutes, lookbackDays, minDropPercent, takeProfitPercent, stopLossPercent, next_bar_entry = TRUE) {
  ...
  idx_start <- NROW(dat) - min_len + 1
  hh_prev <- hh_prev[(length(hh_prev) - min_len + 1):length(hh_prev)]
  L <- L[idx_start:NROW(dat)]
  O <- O[idx_start:NROW(dat)]
  C <- C[idx_start:NROW(dat)]
...
simulate_nav <- function(O, C, buy, sell, init_equity = 100000, fee = 0.001, next_bar_entry = TRUE) {
  ...
  for (t in 2:n) {
    ...
    if (!in_trade && buy[t] == 1L) {
      ...
      # 在入场那一刻扣除手续费
      if (entry_idx <= n) {
        # 保证过程：从 t 到 entry_idx 之前 NAV 保持不变
...
parallel::clusterEvalQ(cl, { suppressMessages(library(xts)); suppressMessages(library(data.table)); suppressMessages(library(pbapply)); })
