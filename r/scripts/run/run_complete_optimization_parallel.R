# PEPEUSDT完整参数优化执行脚本 - 真正并行版
#
# 功能：
# 1. 使用修复后的回测函数（资金复利、手续费、边界条件全部修复）
# 2. 真正的32核并行优化（parLapply）
# 3. 0.075%手续费
# 4. 新参数网格：95,220个组合 × 4个时间框架 = 380,880个测试
# 5. 预计执行时间：30-45分钟（32核并行）
#
# 作者：Claude Code
# 日期：2025-10-26
...
backtest_single_param <- function(param_idx, symbol_data, param_grid, tf_minutes) {
...
for (i in 1:length(pepe_symbols)) {
  ...
  results_list <- parLapply(cl, 1:nrow(param_grid), function(idx) {
    backtest_single_param(idx, symbol_data, param_grid, tf_minutes)
  })
...
cat(sprintf("实际并行加速比: %.1fx\n",
            (nrow(all_results_df) * 0.20 / 60) / elapsed_time))
