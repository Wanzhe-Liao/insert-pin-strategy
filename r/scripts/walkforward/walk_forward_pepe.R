...
option_list <- list(
  make_option(c("-d", "--dataset"), type="character", default="PEPEUSDT_5m",
              help="Dataset name (e.g., PEPEUSDT_5m, DOGEUSDT_15m) [default: %default]"),
  make_option(c("-n", "--train_months"), type="integer", default=1,
              help="Training window size in months (1, 2, or 3) [default: %default]"),
...
cat('\n')
cat('════════════════════════════════════════════════════════════════════════════\n')
cat(sprintf('%s Walk-Forward Analysis (滚动回测)\n', dataset_name))
cat('════════════════════════════════════════════════════════════════════════════\n\n')
cat(sprintf('数据集: %s\n', dataset_name))
cat(sprintf('训练窗口: %d个月\n', train_months))
cat(sprintf('并行核心: %d\n', n_cores))
cat(sprintf('输出目录: %s\n\n', output_dir))
...
generate_rolling_windows <- function(month_ids, train_size, test_size=1) {
  ...
  cat(sprintf('OK 生成 %d 个滚动窗口\n', length(windows)))

  return(windows)
}

quick_optimize_2stage <- function(train_data, cores=32) {
...
walk_forward_backtest <- function(full_data, train_months, cores=32) {
  month_list <- split_data_by_month(full_data)

  month_ids <- names(month_list)

  windows <- generate_rolling_windows(month_ids, train_months, test_size=1)
  ...
  for (w in windows) {
    ...
    cat('────────────────────────────────────────────────────────────────────────────\n')
    cat(sprintf('[Window %d/%d] 训练: %s | 测试: %s\n',
    ...
    cat(sprintf('训练数据: %d根K线\n', nrow(train_data)))
    cat(sprintf('测试数据: %d根K线\n\n', nrow(test_data)))

    cat('阶段1: 训练期参数优化...\n')
    opt_start <- Sys.time()
    opt_result <- quick_optimize_2stage(train_data, cores=cores)
...
results_df <- walk_forward_backtest(full_data, train_months, cores=n_cores)
...
detail_file <- file.path(output_dir, sprintf('%s_walkforward_n%d_details.csv', dataset_name, train_months))
write.csv(results_df, detail_file, row.names = FALSE)
cat(sprintf('OK 详细结果已保存: %s\n', detail_file))

summary_file <- file.path(output_dir, sprintf('%s_walkforward_n%d_summary.txt', dataset_name, train_months))
sink(summary_file)
cat(sprintf('Training Window: %d months\n\n', train_months))
cat('综合绩效指标\n')
cat('────────────────────────────────────────────────────\n')
cat(sprintf('累积收益率:      %.2f%%\n', summary$cumulative_return_pct))
cat(sprintf('平均胜率:        %.1f%%\n', summary$avg_winrate_pct))
