...
#
# 关键差异分析：
# - TradingView: 9笔交易, 100%胜率, 175.99%收益
# - R原版: 127笔交易, 58%胜率, 318.56%收益
#
# 原因：
# - 持仓期间继续入场（违反一次只一个持仓原则）
# - 使用收盘价判断出场（错过盘中触发的止盈止损）
# - 使用收盘价执行出场（价格滑点）
#
# ============================================================================
...
#'
#' 规则（对齐Pine Script）：
#' 1. 计算过去N天的最高价（不包含当前K线）
#' 2. 当前K线的最低价与窗口最高价比较
#' 3. 如果跌幅 >= minDrop，产生买入信号
#'
#' Pine Script等价代码：
#' ```pine
#' lookbackBars = lookbackDays * barsPerDay
#' windowHigh = ta.highest(high, lookbackBars)[1]  // [1]表示排除当前K线
#' currentLow = low
#' dropPercent = (windowHigh - currentLow) / windowHigh * 100
  ...
  for (i in 1:n) {
    ...
    if (inPosition && i > entryBar) {
      ...
      if (!is.na(currentHigh) && !is.na(currentLow) &&
        ...
        # FIX 测试假设：process_orders_on_close=true 可能只使用Close价格检查止损
        # 原逻辑：使用High/Low盘中触发
        # hitTP <- currentHigh >= tpPrice
        # hitSL <- currentLow <= slPrice

        # 新逻辑：仅使用Close价格触发（对齐TradingView可能的行为）
...
cat("  OK 使用High/Low盘中触发止盈止损\n")
cat("  OK 使用精确的TP/SL价格执行\n")
cat("  OK 记录所有被忽略的信号\n")
cat("  OK 详细的交易日志和调试信息\n")
cat("\n")
